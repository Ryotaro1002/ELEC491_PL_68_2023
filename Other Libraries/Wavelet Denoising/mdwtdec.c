...
static void dwtLOC(const double x_data[], double a_data[], int a_size[2],
                   double d_data[], int d_size[2], double sz[2])
{
  static double tmp_data[171];
  static double y_data[164];
  double b_y_data[164];
  int k;
  unsigned char I_data[164];
  memset(&I_data[0], 0, 164U * sizeof(unsigned char));
  for (k = 0; k < 7; k++) {
  ...
  for (k = 0; k < 7; k++) {
    I_data[k + 157] = (unsigned char)(150 - k);
  }
  for (k = 0; k < 164; k++) {
    double d;
    d = x_data[I_data[k] - 1];
  ...
  conv2NonSeparable(b_y_data, 164, tmp_data, &k);
  a_size[1] = 78;
  a_size[0] = 1;
  for (k = 0; k < 78; k++) {
    a_data[k] = tmp_data[(k << 1) + 8];
  }
  memcpy(&b_y_data[0], &y_data[0], 164U * sizeof(double));
  b_conv2NonSeparable(b_y_data, 164, tmp_data, &k);
  d_size[1] = 78;
  d_size[0] = 1;
  for (k = 0; k < 78; k++) {
 ...
 * Return Type  : void
 */
void mdwtdec(const double x[150], char *dec_dirDec, double *dec_level,
             ...
             cell_wrap_4 dec_cd_data[], int dec_cd_size[2])
{
  ...
  static double a__1_data[171];
  static double a__2_data[171];
  static double b_y_data[164];
  ...
  memset(&I_data[0], 0, 92U * sizeof(signed char));
  for (loop_ub = 0; loop_ub < 7; loop_ub++) {
    I_data[loop_ub] = (signed char)(7 - loop_ub);
  }
  for (loop_ub = 0; loop_ub < 78; loop_ub++) {
    I_data[loop_ub + 7] = (signed char)(loop_ub + 1);
  }
  for (loop_ub = 0; loop_ub < 7; loop_ub++) {
    I_data[loop_ub + 85] = (signed char)(78 - loop_ub);
  }
  for (i = 0; i < 92; i++) {
    double d;
  ...
  b_conv2NonSeparable(b_y_data, 92, a__1_data, &loop_ub);
  i = dec_cd_data[1].f1->size[0] * dec_cd_data[1].f1->size[1];
  dec_cd_data[1].f1->size[1] = 42;
  ...
  dec_dataSize[0] = 150.0;
  dec_dataSize[1] = 1.0;
  memcpy(&b_y_data[0], &y_data[0], 92U * sizeof(double));
  conv2NonSeparable(b_y_data, 92, a__1_data, &loop_ub);
  i = tempcD->size[0] * tempcD->size[1];
  tempcD->size[1] = 1;
  tempcD->size[0] = 42;
  emxEnsureCapacity_real_T(tempcD, i);
