...
static void threadA( void *pvParameters ) 
{
 ...
 while(1){
    //Serial.println("Thread A: Started"+String(z_idx));
      ...
      //Serial.println("Thread B: Startedddddddddddddddddddddddddddddddd");
...
static void threadB( void *pvParameters ) 
{
  ...
  while(1)
  {
    ...
    if (isBuffer0Full) {
      ...
      Serial.println("Error case: neither buffer is full");
      while(1){};
    }
    ...
    //Serial.println("Denoising begin");
    wDenoise(input, denoised);
    //Serial.println("Denoising end");
...
static void threadC( void *pvParameters ) 
{
  ...
  while(1)
  {
      ...
      Serial.println("Thread C: start");
      Serial.println(count);
      Serial.println(fallllll);
      ...
      if ((count+check_count) != check_count){
        ...
        vTaskSuspend(NULL);
       //vTaskDelayUntil(&lastWakeTimeeee, 720000);
...
static void threadD( void *pvParameters ) 
{
 ...
 const float fallThreshold = 0.5; // Adjust this value based on your requirements
  ...
  while (1) {
      //Serial.println("Thread DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD");
      float32_t latestYValue = y * ADXL343_MG2G_MULTIPLIER;
       //float32_t latestXValue = x * ADXL343_MG2G_MULTIPLIER;
        //float32_t latestZValue = z * ADXL343_MG2G_MULTIPLIER;
     // Serial.println(latestXValue);
     // Serial.println(latestYValue);
      //Serial.println(latestZValue);
         ...
         if (latestYValue < fallThreshold) {
            ...
            while (1) {
               ...
               vTaskSuspend(Handle_cTask);
                   ...
                   ThingSpeak.begin(client); 
                   Serial.println("FAILLLLLLLLLL EMAIL");
   ...
   //Serial.println(check_fall);     
   //Serial.println("Thread DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD");
   vTaskDelayUntil(&lastWakeTimee, 40000);
  }
...
//*****************************************************************
  ...
  xTaskCreate(threadD,     "Task D",       256, NULL, tskIDLE_PRIORITY + 2, &Handle_dTask);
