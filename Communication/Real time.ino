...
static void threadC( void *pvParameters ) 
{
    ...
    Serial.println("Loop begins" + String(count));
    
    int x = ThingSpeak.writeField(tempChannelNumber, TestFieldNumber, collisiondata, myWriteAPIKey);
...
//*****************************************************************
  ...
  xTaskCreate(threadC,     "Task C",       256, NULL, tskIDLE_PRIORITY + 3, &Handle_cTask);
...
void gettime(){
  ...
  */
  ...
  check_collision[count] = collisiondata;
  count = count+1;
