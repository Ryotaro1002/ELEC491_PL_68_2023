...
void wDenoise(float32_t *input, float32_t *output) {
    ...
    wDecomp(approx2, detail3, approx3, BLOCK_SIZE/4);
    // level 4 wavelet decomposition
    wDecomp(approx3, detail4, approx4, BLOCK_SIZE/8);
    // detail coefficients thresholding
    softThr(detail1, BLOCK_SIZE/2);
    softThr(detail2, BLOCK_SIZE/4);
    softThr(detail3, BLOCK_SIZE/8);
    softThr(detail4, BLOCK_SIZE/16);
    // delay compensation 
    delayComp(detail1, BLOCK_SIZE/2, DELAY1);
    delayComp(detail2, BLOCK_SIZE/4, DELAY2);
    delayComp(detail3, BLOCK_SIZE/8, DELAY3);
    // level 4 wavelet reconstruction
    wReconst(detail4, approx4, out4, BLOCK_SIZE/16);
    // level 3 wavelet reconstruction
    wReconst(detail3, out4, out3, BLOCK_SIZE/8);
    // level 2 wavelet reconstruction
    wReconst(detail2, out3, out2, BLOCK_SIZE/4);
