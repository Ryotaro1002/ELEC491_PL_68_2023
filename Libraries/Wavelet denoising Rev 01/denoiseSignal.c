...
/*
 * Arguments    : const double input[128]
 *                double out_data[]
 *                int out_size[1]
 * Return Type  : void
 */
void denoiseSignal(const double input[128], double out_data[], int out_size[1])
{
  static double sigmahat_data[134];
  ...
  emxArray_real_T *tmp;
  emxArray_real_T *txden;
  emxArray_real_T *x;
  double b_sigmahat_data[134];
  ...
  xtmp = sqrt(2.0 *
              log((int)((double)(a__1_data_idx_0 + a__1_data_idx_1) + 37.0)));
  ...
  for (jj = 0; jj < 2; jj++) {
    ...
    for (i = 0; i < b_acoef; i++) {
      x_data[i] = temp_cD_dirDec1.data[jj].f1->data[i];
    }
    if (cv[(int)ThresholdRule_cal] == 's') {
      ...
      for (k = 0; k < i; k++) {
        for (b_k = 0; b_k < i1; b_k++) {
          xtmp = x_data[b_k + x->size[1] * k];
          if (rtIsNaN(xtmp)) {
            x_data[b_k + x->size[1] * k] = rtNaN;
          } else if (xtmp < 0.0) {
            x_data[b_k + x->size[1] * k] = -1.0;
          } else {
            x_data[b_k + x->size[1] * k] = (xtmp > 0.0);
          }
        }
      }
      i = a->size[0] * a->size[1];
    ...
    b_acoef = x->size[1] * x->size[0];
    for (i = 0; i < b_acoef; i++) {
      xdecdenoised_cd.data[jj].f1->data[i] = x_data[i];
    }
  }
  emxFree_real_T(&a);
  ...
  for (varargin_3 = 0; varargin_3 < 2; varargin_3++) {
    ...
    if ((temp_cD->size[1] == xdecdenoised_cd.data[varargin_3].f1->size[0]) &&
      ...
      b_acoef = xdecdenoised_cd.data[varargin_3].f1->size[1] *
                xdecdenoised_cd.data[varargin_3].f1->size[0];
      for (i = 0; i < b_acoef; i++) {
        temp_cD_dirDec1.data[varargin_3].f1->data[i] = x_data[i];
      }
    } else {
      i = 0;
      i1 = 0;
      b_bcoef = 0;
      bcoef = 0;
      varargin_2 = temp_cD_dirDec1.data[varargin_3].f1->size[0] *
                   temp_cD_dirDec1.data[varargin_3].f1->size[1];
      temp_cD_dirDec1.data[varargin_3].f1->size[1] =
          xdecdenoised_cd.data[varargin_3].f1->size[0];
      temp_cD_dirDec1.data[varargin_3].f1->size[0] =
          xdecdenoised_cd.data[varargin_3].f1->size[1];
      emxEnsureCapacity_real_T(temp_cD_dirDec1.data[varargin_3].f1, varargin_2);
      for (varargin_2 = 0; varargin_2 < acoef * b_acoef; varargin_2++) {
        temp_cD_dirDec1.data[varargin_3]
            .f1->data[i1 + temp_cD_dirDec1.data[varargin_3].f1->size[1] * i] =
            x_data[bcoef + temp_cD->size[1] * b_bcoef];
        i++;
        b_bcoef++;
  ...
  emxInit_real_T(&txden, 2);
  i = txden->size[0] * txden->size[1];
  txden->size[1] = temp_x->size[0];
  txden->size[0] = temp_x->size[1];
  emxEnsureCapacity_real_T(txden, i);
  x_data = txden->data;
  b_acoef = temp_x->size[1];
  for (i = 0; i < b_acoef; i++) {
    acoef = temp_x->size[0];
    for (i1 = 0; i1 < acoef; i1++) {
      x_data[i1 + txden->size[1] * i] = temp_x_data[i + temp_x->size[1] * i1];
    }
  }
  ...
  i = 0;
  i1 = 0;
  out_size[0] = txden->size[0];
  for (b_bcoef = 0; b_bcoef < acoef; b_bcoef++) {
    out_data[b_bcoef] = x_data[i1 + txden->size[1] * i];
    i++;
    if (i > txden->size[0] - 1) {
      i = 0;
      i1++;
